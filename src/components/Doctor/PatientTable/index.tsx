                ...
                {patientList && patientList.length > 0 ? (
                  patientList.map((patient) => (
                    <TableRow key={patient.id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                      ...
                      <TableCell align="right">{patient.opd}</TableCell>
                      <TableCell align="right">{patient.idp}</TableCell>
                      <TableCell align="right">{patient.name}</TableCell>
                      <TableCell align="right">{patient.surname}</TableCell>
                      <TableCell align="right">
                        <IconButton onClick={() => navigate(getDetailPath(PATIENTS_PATH, patient.id))}>
                          <ArrowForwardIos />
                        </IconButton>
                      </TableCell>
                    </TableRow>
